...
int Input::GetInteger(Output *pO) const 
{
	...
	while (1) 
	{
		pWind->WaitKeyPress(Key);
		if (Key == 27)	                // ESCAPE key is pressed
			return 0 ;	           // returns nothing as user has cancelled label (TO BE EDITED)
		if (Key == 13)	               // ENTER key is pressed
			return stoi(integer);
		if ((Key == 8) && (integer.size() >= 1))	// BackSpace is pressed
			integer.resize(integer.size() - 1);
		else if(Key == 45 && integer.size() == 0)
			integer += Key;
		else if (Key >=48 && Key <= 57)
			integer += Key;
		if (pO)
			pO->PrintMessage(integer);
	}
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			x = x / UI.CellWidth;                    //NumHorizontalCells(11)  1210 UI.CellWidth----->110
			y = (y-UI.ToolBarHeight)/ UI.CellHeight; //NumVerticalCells(9)     495  UI.CellHeight----->55
			       
			cellPos.SetVCell(y);                     //integrate with CellPosition.cpp and check for 9/11 case
			cellPos.SetHCell(x);
