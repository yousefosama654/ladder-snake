...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	return UI.ToolBarHeight+cellPos.VCell()*UI.CellHeight; // this line should be changed with your implementation
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\delete_card.jpg";
	MenuItemImages[ITM_SAVE_GRID] = "images\\save_card.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\input_dice.jpg";
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	if (cardNum == -1 || cellNum ==1 || cellNum==99) // no card
	...
	if (cardNum != -1 && cellNum != 1 && cellNum != 99) // Note: cardNum -1 means no card
...
int numofhorizontallines=(toCell.GetCellNum()-fromCell.GetCellNum())/11;
for(int i=1;i<=numofhorizontallines;i++){pWind->SetPen(UI.LadderColor,UI.LadderlineWidth);
int ytemp=toStartY+i*UI.CellHeight;
pWind->DrawLine(x12,ytemp,x34,ytemp,FRAME);
}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	int y[4]={y1+yChange,y1,y1-yChange,y1};
