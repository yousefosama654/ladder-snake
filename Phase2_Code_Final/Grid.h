...
public:
	...
	GameObject * getGameObject(const CellPosition & position);
	Player* GetNextPlayer(const CellPosition& position, Player*); // Gets a Pointer to the first player after the passed "position"
	...
	bool CheckOverLapping(GameObject* game_object) const;
	void Clear();
	void SaveAll(ofstream& outfile, object_type type);
